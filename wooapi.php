...
$consumer_key = 'ck_1f2b2b4019b854e6b346585931af6ea7'; // Add your own Consumer Key here
$consumer_secret = 'cs_8936e72aefea92c979f81306f4a23662'; // Add your own Consumer Secret here
...
if(isset($_GET["endpoint"])){
	...
	switch($endpoint){
		...
		case "products": echo  $wc_api->get_products(); break;
		case 'product':	echo $wc_api->get_product($_GET["id"]); break;
		case "orders": echo "orders!"; break;
		case "custom":	echo $wc_api->make_custom_endpoint_call('products'); break;
		case "categories":	echo $wc_api->get_categories(); break;
		case "productsbycategory" : echo $wc_api->getProductsByCategory($_GET['cat']); break;
	...
	echo "You where expecting some kind of documentation, weren't you? ;)<br>Don't worry, we will write it shortly.<br>Farewell!";
}
...
class WC_API_Client {
	...
	public function get_categories(){
		...
		$sql = "SELECT DISTINCT t.term_id, t.name, t.slug, tt.taxonomy
				FROM wp_term_taxonomy tt, wp_terms t
				WHERE tt.taxonomy = 'product_cat'
				AND tt.term_taxonomy_id = t.term_id";
		$result = $mysqli->query($sql);
		...
		while($row = $result->fetch_array()){
			...
			$thumbnail_id = get_woocommerce_term_meta( $row['term_id'], 'thumbnail_id', true );
			$parent = get_ancestors( $row['term_id'], 'product_cat' );

			if(count($parent)>0)	$data[$i]['parent'] = $parent[0];
			else 					$data[$i]['parent'] = 0;

			$data[$i]['term_id'] = $row['term_id'];
			$data[$i]['name'] = utf8_decode($row['name']);
			$data[$i]['slug'] = $row['slug'];
			$data[$i]['taxonomy'] = $row['taxonomy'];
			$data[$i]['image'] = utf8_decode( wp_get_attachment_url( $thumbnail_id ));
			$i++;
		}
		
		$datajson = '{"categories": '.json_encode($data).' }';
		return $datajson;
	}
	...
	private function _make_api_call( $endpoint, $params = array(), $method = 'GET' ) {
		...
		if ( $this->_return_as_object ) {
			...
			//Added by Darknoe to scape special characters and new lines in json (15/09/2014)
